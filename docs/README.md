...
- 고객에게 적용되는 `혜택 내역` 판별 기능 - EventManager 클래스
  ...
  - [X] 총주문 금액 10,000원 이상부터 이벤트가 적용되도록 한다.
...
- [X] `할인 후 예상 결제 금액`을 계산한다.(할인 전 총 주문금액-총혜택 금액)
- [X] 총 혜택 금액에 따라 `12월 이벤트 배지`를 선정한다.
  - 5천원 이상=별, 1만원이상=트리, 2만원이상=산타

- [X] 이벤트 참여 고객의 숫자를 구하는 기능
...
-  주문 메뉴와 개수를 입력받는 기능
    ...
    - [X] 예외상황: 메뉴 입력시 '-'를 입력하지 않은 경우, 에러메세지를 출력한다. (메뉴 형식 예외처리)
...
- 각각의 이벤트 혜택 출력 기능
    - [X] `할인 전 총 주문 금액`을  출력한다.
      - [X] 금액 출력 시 , 로 정리한다.
    - [X] `증정 메뉴`를 출력한다.
      - 총 주문 금액이 12만원 이상이면 "샴페인 1개", 이하면 "없음"을 출력한다.
    - [X] 고객에게 적용되는 `혜택 내역`을 출력한다.
      - [X] 총주문 금액이 10,000원 이하인 경우 "없음"을 출력한다.
    - [X] `총혜택 금액`을 출력한다.
    - [X] `할인 후 예상 결제 금액`을 출력한다.
    - [X] `12월 이벤트 배지`를 출력한다.
      - [X] 총혜택 금액이 5,000원 미만인 경우 "없음"을 출력한다.
